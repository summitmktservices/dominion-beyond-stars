...
try{for(const scenario of selected){
 ...
 const collapseTelemetry=new Map<number,any>();
 const sampleCollapse=()=>{for(const e of g.empires){const active=(g.crises??[]).filter((x:any)=>!x.resolved&&x.empire===e.id),systemic=active.filter((x:any)=>["plague","technological-collapse","economic-collapse","famine","infrastructure-collapse","institutional-failure","disaster","corruption"].includes(x.kind)),load=systemic.reduce((a:number,x:any)=>a+(x.severity??0),0),capacity=e.stateCapacity??70,strain=e.collapseStrain??0,prev=collapseTelemetry.get(e.id)??{id:e.id,name:e.name,minStateCapacity:capacity,maxCollapseStrain:strain,maxSystemicLoad:load,maxSimultaneousSystemicCrises:systemic.length,worstTurn:g.turn,worstCrises:systemic.map((x:any)=>x.kind)};prev.minStateCapacity=Math.min(prev.minStateCapacity,capacity);prev.maxCollapseStrain=Math.max(prev.maxCollapseStrain,strain);prev.maxSimultaneousSystemicCrises=Math.max(prev.maxSimultaneousSystemicCrises,systemic.length);if(load>prev.maxSystemicLoad){prev.maxSystemicLoad=load;prev.worstTurn=g.turn;prev.worstCrises=systemic.map((x:any)=>x.kind)}collapseTelemetry.set(e.id,prev)}};sampleCollapse();
 for(let i=0;i<turns;i++){
  const turnStarted=Date.now();g=tick(g);sampleCollapse();const issues=validateGame(g);
