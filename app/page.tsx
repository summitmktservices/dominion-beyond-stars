...
type Star={id:number;name:string;x:number;y:number;neighbors:number[];owner:number|null;surveyed:boolean;richness:number;habitable:boolean;population:number;station:boolean;fort:number;defense:number;occupiedBy:number|null;devastation:number};
...
type War={a:number;b:number;goal:number|null;exhaustionA:number;exhaustionB:number;start:number};\ntype Game={turn:number;stars:Star[];empires:Empire[];fleets:Fleet[];wars:War[];player:number;log:string[];selected:number|null;selectedFleet:number|null;nextFleet:number};
...
function generate(count=900,ai=31):Game{
 const stars:Star[]=[];
 for(let i=0;i<count;i++)stars.push({id:i,name:nm(),x:40+Math.random()*1520,y:40+Math.random()*1120,neighbors:[],owner:null,surveyed:false,richness:1+rn(5),habitable:Math.random()<.17,population:0,station:false,fort:0,defense:0,occupiedBy:null,devastation:0});
 ...
 return{turn:1,stars,empires,fleets,wars:[],player:0,log:["The interstellar age begins. Your first fleet awaits orders."],selected:empires[0].systems[0],selectedFleet:0,nextFleet:empires.length};
}
...
function battle(g:Game,system:number){
 ...
 for(let i=0;i<here.length;i++)for(let j=i+1;j<here.length;j++){
  ...
  const mod=(x:Fleet,y:Fleet)=>x.design.weapon==="energy"&&y.design.armor==="shield"?1.2:x.design.weapon==="kinetic"&&y.design.armor==="plating"?1.2:x.design.weapon==="missile"&&y.design.armor==="balanced"?1.15:1;\n  const ap=power(a,g.empires[a.owner].tech)*mod(a,b)*(0.8+Math.random()*.4),bp=power(b,g.empires[b.owner].tech)*mod(b,a)*(0.8+Math.random()*.4);
 ...
 if(owners.length===1){const w=owners[0],s=g.stars[system];if(s.owner!==null&&s.owner!==w&&rel(g.empires[w],s.owner).status==="war"){if(s.station||s.fort>0){const attacker=survivors.find(f=>f.owner===w);if(attacker){const dmg=Math.max(5,power(attacker,g.empires[w].tech)-s.fort*8);s.defense=Math.max(0,s.defense-dmg);s.devastation=Math.min(100,s.devastation+10);if(s.defense===0){s.station=false;s.fort=0;g.log.unshift(`Defenses at ${s.name} have fallen.`)}}}else if(s.population===0)capture(g,w,system)}}
}
...
function tick(g:Game):Game{
 ...
 for(const e of ng.empires){
  ...
  if(e.alloys>=45&&myF.length<Math.max(2,Math.ceil(e.systems.length/5))){e.alloys-=45;ng.fleets.push({id:ng.nextFleet++,owner:e.id,name:nm()+" Fleet",system:e.systems[rn(e.systems.length)],ships:{corvette:5+rn(4),frigate:rn(3),cruiser:e.tech>=3?rn(2):0},hp:100,army:2+rn(4),design:{weapon:["kinetic","energy","missile"][rn(3)] as "kinetic"|"energy"|"missile",armor:["plating","shield","balanced"][rn(3)] as "plating"|"shield"|"balanced"}})}
 ...
 for(const w of ng.wars){w.exhaustionA=Math.min(100,w.exhaustionA+.35);w.exhaustionB=Math.min(100,w.exhaustionB+.35)}\n if(ng.turn%5===0)ng.log.unshift(`Turn ${ng.turn}: ${ng.fleets.length} active fleets operate across the galaxy.`);
 localStorage.setItem("dominion-save-v3",JSON.stringify(ng));return ng;
}

export default function Home(){
 const [game,setGame]=useState<Game|null>(null),[tab,setTab]=useState("Galaxy"),[dip,setDip]=useState<number|null>(null);
 useEffect(()=>{const s=localStorage.getItem("dominion-save-v3");setGame(s?JSON.parse(s):generate())},[]);
 ...
 const act=(fn:(g:Game)=>void)=>setGame(old=>{const n=structuredClone(old!) as Game;fn(n);localStorage.setItem("dominion-save-v3",JSON.stringify(n));return n});
 ...
 const buildFleet=()=>act(g=>{const e=g.empires[0];if(e.alloys>=45){e.alloys-=45;g.fleets.push({id:g.nextFleet++,owner:0,name:`Task Force ${g.nextFleet}`,system:e.systems[0],ships:{corvette:6,frigate:e.tech>=2?2:0,cruiser:e.tech>=3?1:0},hp:100,army:3,design:{weapon:"kinetic",armor:"balanced"}});g.log.unshift("A new task force entered service.")}});
 ...
 const diplomacy=(id:number,kind:"improve"|"rival"|"war"|"peace"|"alliance")=>act(g=>{const a=g.empires[0],b=g.empires[id],r=rel(a,id),br=rel(b,0);if(kind==="improve"&&a.influence>=5){a.influence-=5;r.score=Math.min(100,r.score+20);br.score=r.score;g.log.unshift(`Relations improved with ${b.name}.`)}if(kind==="rival"){r.status="rival";br.status="rival";r.score-=20}if(kind==="war"){r.status="war";br.status="war";const goal=r.claims[0]??null;if(!g.wars.some(w=>(w.a===0&&w.b===id)||(w.a===id&&w.b===0)))g.wars.push({a:0,b:id,goal,exhaustionA:0,exhaustionB:0,start:g.turn});g.log.unshift(`War declared on ${b.name}${goal!==null?` for ${g.stars[goal].name}`:""}.`)}if(kind==="peace"&&r.status==="war"){r.status="peace";br.status="peace";g.log.unshift(`Peace concluded with ${b.name}.`)}if(kind==="alliance"&&r.score>=40){r.status="alliance";br.status="alliance";g.log.unshift(`Alliance formed with ${b.name}.`)}});
 ...
 return <main>
  ...
  </div></div></div><div className="card"><h3>{selected?.name||"Select a system"}</h3>{selected&&<><p className="muted">Owner: {selected.owner===null?"Unclaimed":game.empires[selected.owner].name}<br/>Richness: {selected.surveyed?selected.richness:"Unknown"} · Habitable: {selected.surveyed?(selected.habitable?"Yes":"No"):"Unknown"}<br/>Population: {selected.population} · Station: {selected.station?"Yes":"No"}<br/>Fortification: {selected.fort} · Defense: {selected.defense} · Devastation: {selected.devastation}%</p><div className="actions"><button onClick={survey}>Survey</button><button onClick={claim}>Claim</button><button onClick={station}>Build Station</button><button onClick={colonize}>Colonize</button><button onClick={makeClaim}>Fabricate Claim</button><button onClick={fortify}>Fortify</button><button onClick={bombard}>Bombard</button><button onClick={invade}>Invade</button></div>{sf&&game.stars[sf.system].neighbors.includes(selected.id)&&<button style={{marginTop:8,width:"100%"}} onClick={()=>move(selected.id)}>Move {sf.name} here</button>}</>}</div></div>}
